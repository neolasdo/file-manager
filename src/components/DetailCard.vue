  ...
  <v-card class="mx-auto detail-card" tile>
    <v-container fluid class="pa-0" style="height: 100%">
      ...
      <v-tabs v-model="tab" style="height: calc(100% - 64px);">
        ...
        <v-tab
          :href="'#comments'"
          v-if="selectedFiles.length === 1 && $permissions.comment"
          @click="getComments()"
        >
          <v-tooltip bottom>
            <template v-slot:activator="{ on }">
        ...
        <v-tab
          :href="'#clipboard'"
          v-if="clipboard.files.length + clipboard.folders.length"
        ...
        <v-tabs-items v-model="tab" class="tab-content">
          <v-tab-item value="detail" class="tab-item">
            <v-card-text>
              <v-row>
                <v-col
                  ...
                  v-if="
                    !selectedFiles.length &&
                      !selectedFolder.hasOwnProperty('id')
                  "
                >
                  <b v-if="!current.id">{{ $trans("no_item_selected") }}</b>
                  <div
                    v-for="item in $pluginConfig.detailConfig.folderDetail"
                    :key="item.key"
                    v-else
                  >
                    <span
                ...
                <v-col cols="12" v-if="selectedFiles.length === 1">
                  <div
                    v-for="item in $pluginConfig.detailConfig.fileDetail"
                    :key="item.key"
                  >
                    <span
                      class="body-1 font-weight-medium"
                      v-if="selectedFiles[0][item.key]"
                    >
                      {{
                        item.label +
                          ": " +
                          (item.formatFunc
                            ? item.formatFunc(selectedFiles[0][item.key])
                            : selectedFiles[0][item.key])
                      }}
                    </span>
                  </div>
                </v-col>
          ...
          >
            <FileComments
              ...
              :canAddComment="$permissions.addComment"
              ...
              <v-row>
                <v-col cols="12">
                  ...
                  <v-list dense>
                    <v-list-item-group>
                      <v-list-item
                        v-for="(item, i) in clipboard.files"
                        :key="i"
                      >
                        <v-list-item-content>
                          <v-list-item-title
                ...
                <v-col cols="12" v-if="$permissions.moveFolder">
                  ...
                  <v-list dense>
                    <v-list-item-group>
                      <v-list-item
                        v-for="(item, i) in clipboard.folders"
                        :key="i"
                      >
                        <v-list-item-content>
                          <v-list-item-title
                        ...
                        <v-list-item-action>
                          <v-btn icon small>
                            <v-icon
                              @click="removeFolderInClipboard(i)"
                              color="grey lighten-1"
                              >mdi-close
                            </v-icon>
                          </v-btn>
