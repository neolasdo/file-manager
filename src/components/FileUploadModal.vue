          ...
          <v-container>
            ...
            <v-form ref="form" v-model="valid">
              ...
              <v-combobox
                ...
                :error-messages="validate.files"
                :rules="rules.filesRules"
                append-icon
                @click:prepend-inner="openSelectFile"
                ref="upload_input"
              ...
              <v-checkbox
                ...
                :label="$trans('need_approval')"
                v-if="!$pluginConfig.hideNeedApprovalCheckBox"
              />
              <label
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        filesRules: [
          (v) =>
            this.checkDocumentSize(v) ||
            this.lengthOverloadSizeFilesArr() ||
            this.$trans("error_file_size", {
              size: formatSize(this.$pluginConfig.file_max_size),
            }),
          (v) =>
            this.checkMimeDocuments(v) ||
            this.lengthMimeErrorFilesArr() ||
            this.$trans("error_file_mime"),
        ],
  ...
  methods: {
    ...
    async uploadAll() {
      ...
      let promise = new Promise((resolve, reject) => {
        ...
        this.$fileStore.$axios
          .post(this.$fileStore.$getEndpoint("upload").route, formData, {
            headers: {
            ...
            if (errors.response.data.data) {
              this.validate.files =
                errors.response.data.data[
                  Object.keys(errors.response.data.data)[0]
                ][0];
              this.errors = Object.keys(errors.response.data.data).map(
                (item) => {
    ...
    upload(fileInfo, onUploadProgress) {
      ...
      return this.$fileStore.$axios.post(
        this.$fileStore.$getEndpoint("upload").route,
        formData,
        {
          headers: {
            "Content-Type": "multipart/form-data",
    ...
    removeFile(key) {
      if (!this.uploading) {
        ...
        if (this.overload_size_files.indexOf(key) !== -1) {
          this.overload_size_files.splice(
            this.overload_size_files.indexOf(key),
            1
          );
        }
        if (
          this.errors.length === 0 &&
          this.overload_size_files.length === 0 &&
          this.mime_error_files.length === 0
        ) {
          this.validate.files = "";
          this.valid = true;
        }
