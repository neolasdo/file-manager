    ...
    <v-toolbar color="primary" dark dense>
      <v-text-field dark v-model="keyword" dense outlined hide-details>
            ...
            <v-card tile max-width="450px">
              <v-container>
                ...
                <v-card-actions class="justify-center">
                  <v-btn color="secondary" tile @click="reset">{{
                    this.$trans("reset")
                  }}</v-btn>
                  <v-btn color="primary" tile @click="executeSearch">{{
                    this.$trans("search")
                  }}</v-btn>
                </v-card-actions>
              </v-container>
        ...
        <v-btn
          ...
          small
          slot="append"
          @click="search({ keyword: keyword, filter: filter })"
        >
      ...
      <v-menu
        offset-y
        v-if="$permissions.upload || $permissions.create"
        z-index="11"
      >
        <template v-slot:activator="{ on }">
      ...
      <v-menu
        offset-y
        v-if="
          selectedFiles.length && ($permissions.download || $permissions.delete)
        "
        z-index="11"
      >
        <template v-slot:activator="{ on }">
  ...
  methods: {
    ...
    getAllType() {
      ...
      for (const [key, value] of Object.entries(
        this.$pluginConfig.accept_extensions
      )) {
